...
void AESEncryptor::deriveKeyFromPassword(const std::string &password, std::array<uint8_t, 32> &key) {
    key.fill(0);
...
void AESEncryptor::encrypt(const std::string &src, std::string &password) {
...
void AESEncryptor::decrypt(const std::string &src, std::string &password) {
    ...
    //Read data
    ifs.seekg(0, std::ios::end);
    size_t sizeEncryptedFile = ifs.tellg();
    ifs.seekg(0, std::ios::beg);
